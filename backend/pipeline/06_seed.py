...
def main() -> None:
    ...
    print("\n─── Verification ───")
    with Session(engine) as session:
        n_s = session.exec(select(Sector)).all().__len__()
        n_sc = session.exec(select(SectorScore)).all().__len__()
        n_p = session.exec(select(Poi)).all().__len__()
    print(f"  Sectors: {n_s}  |  Scores: {n_sc}  |  POIs: {n_p:,}")
